...
public class ESCUtil {
	...
	public static byte[] getPrintQRCode2(String data, int size){
		...
		byte[] bytes2 = com.rebelity.plugins.btprinter.utils.BytesUtil.getZXingQRCode(data, size);
		return com.rebelity.plugins.btprinter.utils.BytesUtil.byteMerger(bytes1, bytes2);
	}
	...
	public static byte[] getPrintBarCode(String data, int symbology, int height, int width, int textposition){
		...
		try{
			...
			byte[] barcode;
			if(symbology == 10){
				barcode = com.rebelity.plugins.btprinter.utils.BytesUtil.getBytesFromDecString(data);
			}else{
